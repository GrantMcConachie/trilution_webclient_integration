...
def get_file_contents_and_analyze(content, ref_df=None):
    ...
    j = content
    x = np.array([0])
    for i in range(j["sampleData"]["chromatograms"][0]["metadata"]["chromatogramDataCount"]):
        x = np.append(x, (i + 1) * 0.00166666666666667)
    y = np.array(j["sampleData"]["chromatograms"][0]["chromatogramData"]["data"])
    ...
    info_card = make_sample_info_card(sample_info=j, filename="Standard most recently run on Gilson")

    data_table, differences, ref_df_filtered = calculate_ref_table_and_differences(
        peaks, heights, fwhm, ref_df
    )    
    return data_table, differences, fig, info_card, ref_df_filtered
...
def make_sample_info_card(sample_info, filename):
    ...
    info_card = dbc.Card(
        dbc.CardBody(
            ...
            + [
                html.P([html.B(i), ": ", sample_info["sampleData"]["runData"][0][i]])
                for i in ["sampleListID", "methodName", "runDate"]
            ],
        )
    )
    return info_card
